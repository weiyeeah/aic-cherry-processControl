...
> {
  ...
  override async createCompletions(
    ...
    // @ts-ignore
    const result: OpenAISdkRawOutput = await sdk.chat.completions.create(payload, options)

    if (result instanceof Stream) {
      console.log('API Response (Stream): Logging stream chunks asynchronously.')
      const [logStream, returnStream] = result.tee()

      // Log the stream content asynchronously
      ;(async () => {
        try {
          let fullResponse: any = null // Reconstructed final response object
          const choices: { [index: number]: any } = {} // Map from choice index to a reconstructed choice object
          const toolCalls: { [index: number]: { [toolIndex: number]: any } } = {} // Map from choice index to a map of tool call index to tool call object
          ...
          for await (const chunk of logStream) {
            ...
            for (const choice of chunk.choices) {
              ...
              if (delta.content) {
                choices[index].message.content += delta.content
              }
              if (delta.tool_calls) {
                ...
                for (const toolCallDelta of delta.tool_calls) {
                  ...
                  if (!toolCalls[index][toolIndex]) {
                    toolCalls[index][toolIndex] = {
                      id: '',
                      type: 'function',
                  ...
                  const currentToolCall = toolCalls[index][toolIndex]
          ...
          if (fullResponse) {
            // Assemble the final choices
            for (const index in choices) {
              const choice = choices[index]
              if (toolCalls[index]) {
                choice.message.tool_calls = Object.values(toolCalls[index])
              }
              ...
              fullResponse.choices.push(choice)
            }
            // Sort choices by index
            fullResponse.choices.sort((a: any, b: any) => a.index - b.index)
          }

          console.log('API Response (Full Streamed Data):', JSON.stringify(fullResponse, null, 2))
        } catch (e) {
          console.error('Error logging stream:', e)
        }
      })()
