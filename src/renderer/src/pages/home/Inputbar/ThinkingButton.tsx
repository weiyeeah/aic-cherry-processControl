...
const ThinkingButton: FC<Props> = ({ ref, model, assistant, ToolbarButton }): ReactElement => {
  ...
  const handleOpenQuickPanel = useCallback(
    (event?: React.MouseEvent) => {
      // QuickPanel 有一个全局的 handleClickOutside 事件监听器
      // 当点击 ThinkingButton 时，事件冒泡到 document 级别
      // QuickPanel 检测到点击不在面板内且不在 #inputbar 内，立即关闭面板
      // 事件不会冒泡到 QuickPanel 的点击外部处理器，面板将正常打开并保持显示
  ...
  return (
    <Tooltip placement="top" title={t('assistants.settings.reasoning_effort')} arrow>
      <ToolbarButton type="text" onClick={(e) => handleOpenQuickPanel(e)}>
        {getThinkingIcon()}
      </ToolbarButton>
    </Tooltip>
  )
